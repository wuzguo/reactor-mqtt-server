...
@Slf4j
public class TransportMessageMapper implements TableMapper<TransportMessage> {

    /**
     * 列族
     */
    private final static byte[] COLUMN_FAMILY = TransportMessage.COLUMN_FAMILY.getBytes(StandardCharsets.UTF_8);
    ...
    @Override
    public TransportMessage mapRow(Result result, int rowNum) throws Exception {
    ...
    @Override
    public List<Mutation> mutations(TransportMessage message) throws Exception {
        // rowKey
        Put put = new Put(Bytes.toBytes(message.getRow()));
        put.addColumn(Bytes.toBytes(TransportMessage.COLUMN_FAMILY), ROW, Bytes.toBytes(message.getRow()));
        put.addColumn(Bytes.toBytes(TransportMessage.COLUMN_FAMILY), TOPIC, Bytes.toBytes(message.getTopic()));
        put.addColumn(Bytes.toBytes(TransportMessage.COLUMN_FAMILY), RETAIN, Bytes.toBytes(message.getRetain()));
        put.addColumn(Bytes.toBytes(TransportMessage.COLUMN_FAMILY), QOS, Bytes.toBytes(message.getQos()));
        put.addColumn(Bytes.toBytes(TransportMessage.COLUMN_FAMILY), DUP, Bytes.toBytes(message.getDup()));
        put.addColumn(Bytes.toBytes(TransportMessage.COLUMN_FAMILY), COPY_BYTE_BUF, message.getCopyByteBuf());
